    ...
    public function streamMessage(Conversation $conversation, Request $request)
    {
        ...
        try {
            ...
            $stream = (new ChatService())->streamConversation(
                messages: $messages,
                model: $conversation->model ?? $request->user()->preferred_model ?? ChatService::DEFAULT_MODEL,
            );
            ...
            // 11. Générer un titre automatiquement si c'est le premier échange
            if ($conversation->messages()->count() === 2 && !$conversation->title) {
                // Appel asynchrone pour générer le titre
                dispatch(function () use ($conversation) {
                    app(ConversationController::class)->generateTitle($conversation);
                })->afterResponse();
            }
